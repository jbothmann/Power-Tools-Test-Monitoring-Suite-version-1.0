...
class Test:
    ...
    def __init__(self, url, testNum, title="[title]", subtitle="[subtitle]"):
        ...
        #This button copies the contents of the dataLabel to the user's clipboard
        self.buttonC = Button(self.labelWidgetFrame, text="\U0001F5CD", command=lambda root=root, self=self: root.clipboard_append(self.toString()), width=2)
        self.buttonC.grid(row=0, column=1)
        #Shows when the user's machine has lost connection with the server, or if this station has been deleted from the test manager
        self.connectionHint = Label(self.labelWidgetFrame)
        self.connectionHint.grid(row=0, column=2)
    ...
    def draw(self, r, c):
        ...
        T.apply([self.dataLabel, self.labelWidgetFrame, self.stationNumLabel, self.buttonC, self.connectionHint, self.frame, self.statusIndicator, self.button2])
...
def changeView():
    ...
    def refresh():
        ...
        try:
            ...
            for oo in get.json():
                l.append(T.apply(Label(topFrame, text='Station '+str(oo['number'])+'\n'+str(oo['title'])+'\n'+str(oo['subtitle']), pady=5)))
                r.append(IntVar())
...
def theme():
    ...
    terse = newTheme.apply(Label(topFrame, text="Current Theme: %s, %i %s" % (newTheme.theme.title, newTheme.size, newTheme.family)))
    terse.pack()
    ...
    def refresh():
        ...
        newTheme.apply([tl, topFrame, midFrame, botFrame, terse])
        terse.config(text="Current Theme: %s, %i %s" % (newTheme.theme.title, newTheme.size, newTheme.family))
        newTheme.apply([colorsHeader, fontSizeHeader, fontHeader, saveButton, cancelButton, r, l])
    ...
    for ii in range(len(sizes)):
        ...
        l.append(newTheme.apply(Label(midFrame, text=str(sizes[ii]))))
    ...
    for ii in range(len(families)):
        ...
        l.append(newTheme.apply(Label(midFrame, text=families[ii])))
