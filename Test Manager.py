...
class Test:
    #a separate tuple array to keep track of all comments in chronological order (unused)
    allComments = []
    ...
    def __init__(self, address=-1, name="[test name]", serial="[serial number]", data=None, controls=None):
        ...
        #This button opens up an editing screen for the test
        self.button0 = Button(self.labelWidgetFrame, text="\U00002699", command=lambda: editTests(self.testNum), width=2)
        ...
        self.buttonC.grid(row=0, column=2)
    ...
    def draw(self, r, c):
        ...
        T.apply([self.dataLabel, self.labelWidgetFrame, self.stationNumLabel, self.button0, self.buttonC, self.frame, self.statusIndicator, self.button1, self.button2])
...
def connect():
    ...
    #refresh button
    T.apply(Button(COMframe, text='Rescan COM Ports', command=getCOMs)).grid(row=1, column=1, padx=5, pady=5)
    ...
    disconnectCOMButton.grid(row=0, column=2, padx=5, pady=5)
    ...
    disconnectAPIButton.grid(row=0, column=2, padx=5, pady=5)

    #copy address to clipboard
    copyButton = T(Button(APIframe, text='Copy Address', command=lambda root=root, poll=poll: root.clipboard_append(server.getAddress())))
    copyButton.grid(row=1, column=1, padx=5, pady=5)
...
def deleteTest():
    ...
    for i in range(len(tests)):
        ...
        l.append(T(Label(optionsFrame, text=option)))
        r.append(IntVar())
        r[i].set(0)
...
def changeView():
    ...
    for i in range(len(tests)):
        ...
        l.append(T(Label(topFrame, text=option)))
        r.append(IntVar())
...
def pauseTests():
    ...
    pauseButtons = []
    for oo in tests:
        stationLabels.append(T(Label(midFrame, text="Station "+str(oo.testNum)+": "+oo.name)))
...
def theme():
    ...
    terse = newTheme(Label(topFrame, text="Current Theme: %s, %i %s" % (newTheme.theme.title, newTheme.size, newTheme.family)))
    terse.pack()
    ...
    def refresh():
        ...
        newTheme.apply([saveButton, cancelButton, r, terse, colorsHeader, fontSizeHeader, fontHeader, l])
    ...
    colorsHeader = Label(midFrame, text="Color Theme:")
    ...
    for ii in range(len(families)):
        ...
        l.append(Label(midFrame, text=families[ii]))
...
class Server():
    ...
    def toString(self):
        if self.serving:
            return "Serving on %s" % (self.getAddress())
        else: 
            return "Not Serving"

    #returns the machine's external ip and the selected port
    def getAddress(self):
